...
impl State {
    pub fn new() -> Self {
        Self {
            link: RefCell::new(AblLink::new(100.)),
...
fn print_state(state: &mut State) {
    ...
    let time = state.link.borrow().clock_micros();
    let enabled = match state.link.borrow().is_enabled() {
        true => "yes",
        false => "no ",
    }
    .to_string();
    let num_peers = state.link.borrow().num_peers();
    let start_stop = match state.link.borrow().is_start_stop_sync_enabled() {
        true => "yes",
        false => "no ",
    };
...
fn poll_input(state: &mut State) -> crossterm::Result<()> {
    ...
    if poll(Duration::from_millis(50))? {
        if let Event::Key(event) = read()? {
            state.update_app_state();
            let tempo = state.session_state.tempo();
            let time_stamp = state.link.borrow().clock_micros();
            let enabled = state.link.borrow().is_enabled();
            ...
            match event.code {
                ...
                // Enable Link Toggle
                KeyCode::Char('a') => state.link.borrow_mut().enable(!enabled),
                ...
                KeyCode::Char('s') => state
                    .link
                    .borrow_mut()
                    .enable_start_stop_sync(!state.link.borrow().is_start_stop_sync_enabled()),
...
fn main() {
    ...
    state.link.borrow_mut().enable(false);
}
